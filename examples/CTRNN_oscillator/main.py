...
def evol_CTRNN_osc(fitness_file_name, plot_flag):
    ...
    with tf.Session() as sess:
        ...
        with TFSearch(EVOL_P) as tfs:
            for g in range(EVOL_P["max_gens"]):
                with MakeCTRNN(EVOL_P) as make_net:
                    ...
                    net_weights, net_biases, net_taus = sess.run(
                        [
                            make_net.weight_matrices,
                            make_net.bias_matrices,
                            make_net.tau_matrices,
                        ],
                        feed_dict={pop_pl: this_pop},
                    )
                ...
                with CTRNN(EVOL_P, net_weights, net_biases, net_taus) as net:
                    ...
                    for t in range(int(EVOL_P["transient_duration"])):
                        this_state, this_output = sess.run(
                            [net.step_state, net.step_output],
                            feed_dict={state_pl: this_state, output_pl: this_output},
                        )

                    fs = np.zeros(EVOL_P["pop_size"])
                    for t in range(int(EVOL_P["eval_duration"])):
                        this_state, this_output_p = sess.run(
                            [net.step_state, net.step_output],
                            feed_dict={state_pl: this_state, output_pl: this_output},
                        )

                        # absolute time-difference of neuron 1 in all networks
                        out1_abs_diff = np.reshape(
                            np.fabs(this_output - this_output_p),
                            [EVOL_P["pop_size"], EVOL_P["cns_size"]],
                        )
                    ...
                    # normalizing fitness
                    # fs /= (EVOL_P['eval_duration']/EVOL_P['step_size'])
                    ...
                    this_pop = sess.run(
                        tfs.step_generation, feed_dict={pop_pl: this_pop, fits_pl: fs}
                    )
        ...
        if plot_flag:
            ...
            this_pop = [
                this_pop[0]
            ]  # first individual is always the best because TFSearch sorts them
