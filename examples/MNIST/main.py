...
def main():
    ...
    thisPop = np.random.uniform(size=(EVOL_P['popSize'],EVOL_P['genotypeSize']),
                    low=EVOL_P['genotypeMinVal'],
                    high=EVOL_P['genotypeMaxVal'])
    ...
    with NetworkPop(EVOL_P) as nets:
        for gen in range(EVOL_P['maxGens']/2):
            ...
            fs = sess.run(nets.fitness,{popPl:thisPop,ins:inputs_,ys:y_})
            ...
            thisPop = sess.run(tfs.generationStep,{popPl:thisPop,fitsPl:fs})

    # Change params and continue running
    EVOL_P['mutationVariance'] = EVOL_P['mutationVariance']/2.
    with NetworkPop(EVOL_P) as nets:
        for gen in range(EVOL_P['maxGens']*2):
            inputs_, y_ = mnist.train.next_batch(EVOL_P['numSamples'])
            y_ = [np.argmax(y_,1)]
    ...
    print '\n\nTesting final best...Fitness=',#,fs[0],'\n\n'
    ...
    #weights,biases = sess.run(nets.phenotypes,{popPl:bestIndividual})
