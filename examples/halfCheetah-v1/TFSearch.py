...
class TFSearch(object):
    def __enter__(self):
        return self

    def __init__(self, EVOL_P):
        """
            The TF graph is defined here.
        ...
        with tf.device(self.device), tf.variable_scope("population", reuse=None):
            pop = tf.get_variable(
                "pop",
                shape=(self.pop_size, self.genotype_size),
                initializer=tf.random_uniform_initializer(
                    minval=self.genotype_min_val, maxval=self.genotype_max_val
                ),
            )

        # reading the population variable to use it
        with tf.variable_scope("population", reuse=True):
            self.pop_v = tf.get_variable(
                "pop", shape=(self.pop_size, self.genotype_size), trainable=False
            )

        with tf.name_scope("elitist_selection"):
            # randomize population just to shuffle selecting for individuals of same fitness
            # rand_inds = tf.random_shuffle(np.arange(self.pop_size))
            ...
            self.parents = tf.gather(self.pop_v, tf.slice(ranks, [0], [num_parents]))
        ...
        with tf.name_scope("mutation"):
            ...
            self.crossed_mutant_children = (
                ...
                + tf.random_normal(
                    shape=tf.shape(self.children),
                    mean=0.0,
                    stddev=self.mutation_variance,
                )
            )
        ...
        with tf.name_scope("step_generation"):
            ...
            new_eqiv_pop = tf.slice(
                new_pop, [0, 0], [self.pop_size, self.genotype_size]
            )
            ...
            self.step_generation = tf.assign(self.pop_v, new_bounded_pop)

        tf.summary.scalar("best_fitness", tf.reduce_max(self.fits_pl))
        tf.summary.scalar("avg_fitness", tf.reduce_mean(self.fits_pl))
...
if __name__ == "__main__":
    ...
    print(
        "TFSearch is intended to be imported and used as a package - See README at https://github.com/madvncv/TFSearch"
    )
