...
with tf.name_scope('fitness_evaluation'):
    fitness = tf.reduce_sum(tf.cast(tf.equal(tf.tile(target_pl,[EVOL_P['pop_size'],1]),phenotypes),tf.float32),axis=1)/EVOL_P['genotype_size']
...
# Setting up some writers to visualize the graph
