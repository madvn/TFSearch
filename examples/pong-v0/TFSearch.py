...
class TFSearch(object):
    ...
    def __init__(self,EVOL_P):
        ...
        with tf.device(self.device),tf.variable_scope('population',reuse=None):
            pop = tf.get_variable('pop',shape=(self.pop_size,self.genotype_size),
                                        initializer=tf.random_uniform_initializer(minval=self.genotype_min_val,maxval=self.genotype_max_val))

        # reading the population variable to use it
        with tf.variable_scope('population',reuse=True):
            self.pop_v = tf.get_variable('pop',shape=(self.pop_size,self.genotype_size),trainable=False)
