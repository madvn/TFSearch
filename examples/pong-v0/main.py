...
def preprocess_observation(obs):
    #print 'this obs',np.sum(np.isnan(obs))
    obs = obs[35:194] # crop
    obs = obs[::2,::2,0] # downsample
    obs[obs == 144] = 0 # remove background 1
    obs[obs == 109] = 0 # remove background 2
    obs[obs != 0] = 1 # keep paddles and ball
    obs = obs.flatten()
    return obs
...
def evol_ctrnn_pong(genotype_to_sim=None):
    if EVOL_MODE:
        ...
        with tf.variable_scope('population',reuse=None):
            pop = tf.get_variable('pop',shape=(EVOL_P['pop_size'],EVOL_P['genotype_size']))
    ...
    with tf.Session(config=tf.ConfigProto(log_device_placement=False)) as sess:
        ...
        if not EVOL_MODE:
            ...
            with tf.variable_scope('population',reuse=True):
                sess.run(tf.assign(tf.get_variable('pop'),tf.convert_to_tensor(np.asarray([genotype_to_sim]),dtype=tf.float32)))
        ...
        for gen in range(EVOL_P['max_gens']):
            ...
            sess.run([ctrnns.weight_matrices,
            ...
            for episode_ind in range(EVOL_P['max_num_episodes']):
                ...
                sess.run(ctrnns.randomize_states,{state_pl:np.random.rand(1,EVOL_P['pop_size']*EVOL_P['cns_size'])})
                sess.run(ctrnns.randomize_outputs,{output_pl:np.random.rand(1,EVOL_P['pop_size']*EVOL_P['cns_size'])})
                ...
                while episode_len<EVOL_P['evaluation_duration']:
                    ...
                    # construct output from CTRNNs
                    control_outputs[control_outputs > 0.5] = 2 # move paddle up
                    control_outputs[control_outputs < 0.5] = 3 # move paddle down
                    control_outputs[control_outputs == 0.5] = np.round(np.random.rand())+2
                    ...
                    for ind,env in enumerate(envs):
                        if not_dones[ind]:
                            if not EVOL_MODE:
                                env.render()
                            observation, reward, done, _ = envs[ind].step(int(control_outputs[ind]))
                        else:
            ...
            if EVOL_MODE:
                ...
                _,summary = sess.run([tfs.step_generation,tfs.merged_summary],{fits_pl:rewards})
                ...
                if gen==0 or (gen+1)%50==0:
                    ...
                    with tf.variable_scope('population',reuse=True):
                        np.save('./logs/best_at_'+str(gen+1),sess.run(tf.get_variable('pop'))[0])
